...
// Solution
const checkPromoteds = async (ownerId, promotedValue) => {
  if (promotedValue) {
    try {
...
const create = [
  ...
  // SOLUTION
  check('promoted').custom(async (value, { req }) => { return checkPromoteds(req.user.id, value) }).withMessage('Only one restaurant can be promoted')
]
const update = [
  check('name').exists().isString().isLength({ min: 1, max: 255 }).trim(),
  check('description').optional({ nullable: true, checkFalsy: true }).isString().trim(),
